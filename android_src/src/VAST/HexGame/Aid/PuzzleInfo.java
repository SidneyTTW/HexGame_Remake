...
public class PuzzleInfo {
  ...
  public static int[] minSteps(int type, boolean advanced) {
    ...
    if (!FileProcessor.exists(SourceManagement.PuzzleRecordFiles[type]))
      FileProcessor.createFile(SourceManagement.PuzzleRecordFiles[type],
          SourceManagement.PuzzleCounts[type] * 2);
    int data[] = FileProcessor
        .readData(SourceManagement.PuzzleRecordFiles[type]);
    if (data != null) {
      ...
      int[] result = new int[SourceManagement.PuzzleCounts[type]];
      for (int i = 0; i < SourceManagement.PuzzleCounts[type]; ++i)
        result[i] = 0;
      return result;
    }
  }

  public static boolean testMinSteps(int type, int stage, boolean advanced,
      int steps) {
    if (type < 0 || type >= SourceManagement.PUZZLE_TOTAL_TYPES
    ...
    if (lastSteps <= 0)
      result = true;
    FileProcessor.writeData(SourceManagement.PuzzleRecordFiles[type], stage
        + (advanced ? totalStages(type) : 0), steps);
    return result;
  }

  public static void readColorIndexes(int type, int stage, boolean advanced,
      int[] current, int[] target) {
    InputStream is = SourceManagement.r.openRawResource(SourceManagement
        .puzzleFile(type, stage, advanced));
    try {
