  ...
  public boolean fillBalls(Ball[] balls) {
    ...
    do {
      ...
      while (itr.hasNext()) {
        ...
        if (balls[index] == null)
          balls[index] = new Ball(MathAid.nextUInt()
              % Ball.MAX_VALID_NUMBER_COUNT);
        else
          balls[index].setColor(MathAid.nextUInt()
              % Ball.MAX_VALID_NUMBER_COUNT);
        if (thisLockCount < thisMaxLockNumber) {
          boolean setToLock = (MathAid.nextUInt() % 100) < lockRate * 100;
          balls[index].setLocked(setToLock);
          if (setToLock)
            ++thisLockCount;
        }
      }
    } while (rule.endlessFill()
        && connectionCalculator.hint(balls, gameBoard) < 0);
    return true;
  }
}
