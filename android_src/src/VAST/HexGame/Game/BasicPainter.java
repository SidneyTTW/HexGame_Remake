...
public class BasicPainter {
  ...
  private static void initBallsImages() {
    ImageAid.loadFromFile(SourceManagement.BallFiles, ballsImages,
        ballsFrameCounts);
  }

  private static void initLockImages() {
    lockImages = ImageAid.loadFromFile(SourceManagement.LockFile);
  }

  private static void initBackgroundImages() {
    backgroundImages = ImageAid.loadFromFile(SourceManagement.BackgroundFiles);
  }
  ...
  public static void paintBackGround(int id, MyGraphics graphics, int width,
    ...
    if (id < 0 || id >= SourceManagement.BackgroundFiles.length)
      return;
    ...
    // Draw the image
    ImageAid.drawImageAt(graphics, image, 1.0 * width / image.getWidth(), 1.0
        * height / image.getHeight(), new MyPoint(0, 0), true, false);
  }
